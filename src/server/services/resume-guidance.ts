...
- Formatting markup: format jobs[].description, educations[].description, projects[].description, and personal_details.summary using ONLY this minimal markup: put each achievement on its own line prefixed with "- " to make a bullet; wrap the single most important metric/number in each bullet in **double asterisks** for bold (e.g. "- Increased signups by **32%** ..."). Do not use any other markdown (no headings, italics, links).`;
